...
func (s *Service) CriarConta(ctx context.Context, input CriarContaInput) (*domain.Conta, error) {
    if len(input.Nome) < 3 { return nil, domain.ErrNomeCurto }
    
    cpf, ok := validation.ValidadorCPF(input.CPF)
    if !ok { return nil, domain.ErrCPFINvalido }
    if !validation.ValidadorSenha(input.Senha) { return nil, domain.ErrSenhaCurta }
    if input.Saldo < 10 { return nil, domain.ErrValorInvalido }

    existe, _ := s.repo.VerificarExistePorCPF(ctx, cpf)
    if existe { return nil, domain.ErrCPFJaCadastrado }

    hash, err := validation.HashSenha(input.Senha)
    if err != nil { return nil, fmt.Errorf("erro senha: %w", err) }

    conta := &domain.Conta{
        CPF: cpf, Nome: input.Nome,
        SaldoCentavos: int(input.Saldo * 100),
        SenhaHash: hash, LimiteDiario: 5000000,
    }
...
func (s *Service) Login(ctx context.Context, input LoginInput) (*domain.Conta, error) {
    ...
    if err != nil {
        if errors.Is(err, pgx.ErrNoRows) { return nil, domain.ErrContaNaoEncontrada }
...
func (s *Service) ConsultarSaldo(ctx context.Context, cpf string) (*SaldoOutput, error) {
    conta, err := s.repo.BuscarPorCPF(ctx, cpf)
    if err != nil { return nil, domain.ErrContaNaoEncontrada }
...
func (s *Service) Extrato(ctx context.Context, cpf string) ([]*domain.Transacao, int, error) {
    conta, err := s.repo.BuscarPorCPF(ctx, cpf)
    if err != nil { return nil, 0, domain.ErrContaNaoEncontrada }

    transacoes, err := s.repo.ListarTransacoesPorCPF(ctx, cpf)
    if err != nil { return nil, 0, fmt.Errorf("erro extrato: %w", err) }
...
func (s *Service) FazerPix(ctx context.Context, input PixInput) (*domain.Transacao, error) {
    if input.Valor <= 0 || input.Valor < 1 { return nil, domain.ErrValorInvalido }
    ...
    if err != nil { return nil, domain.ErrDestinatarioNaoEncontrado }
    if input.CPFOrigem == input.CPFDestino { return nil, domain.ErrAutoTransferencia }

    if origem.SaldoCentavos < valorCent { return nil, domain.ErrSaldoInsuficiente }
    ...
    tx := &domain.Transacao{
        Tipo: "TRANSACAO_PIX", ValorCentavos: valorCent,
        CPFRemetente: input.CPFOrigem, CPFDestinatario: input.CPFDestino,
        NomeRemetente: origem.Nome, NomeDestinatario: destino.Nome,
        DataHora: time.Now(),
    }

    s.repo.AtualizarSaldo(ctx, input.CPFOrigem, origem.SaldoCentavos-valorCent)
    s.repo.AtualizarSaldo(ctx, input.CPFDestino, destino.SaldoCentavos+valorCent)
    s.repo.SalvarTransacao(ctx, tx)
...
func (s *Service) Sacar(ctx context.Context, input SaqueInput) (int, error) {
    if input.Valor <= 0 || input.Valor > 2000 { return 0, domain.ErrValorInvalido }
    if int(input.Valor*100)%1000 != 0 { return 0, domain.ErrValorInvalido } // múltiplo de 10

    valorCent := int(input.Valor * 100)
    conta, _ := s.repo.BuscarPorCPF(ctx, input.CPF)
    if conta.SaldoCentavos < valorCent { return 0, domain.ErrSaldoInsuficiente }

    s.repo.AtualizarSaldo(ctx, input.CPF, conta.SaldoCentavos-valorCent)
    s.repo.SalvarTransacao(ctx, &domain.Transacao{
        Tipo: "SAQUE", ValorCentavos: valorCent,
        CPFRemetente: input.CPF, CPFDestinatario: input.CPF,
        DataHora: time.Now(),
    })
    return valorCent, nil
...
func (s *Service) AdicionarChave(ctx context.Context, input AdicionarChaveInput) (*domain.ChavePix, error) {
    ...
    if qtd >= 5 { return nil, domain.ErrLimiteChavesAtingido }

    existe, _ := s.repo.BuscarChavePorValor(ctx, input.Valor)
    if existe != nil { return nil, domain.ErrChaveJaExistente }
...
func (s *Service) RemoverChave(ctx context.Context, input RemoverChaveInput) error {
    ...
    if err != nil { return fmt.Errorf("erro remover: %w", err) }
    return nil
}
...
func (s *Service) MudarSenha(ctx context.Context, input MudarSenhaInput) error {
    ...
    if err != nil { return domain.ErrContaNaoEncontrada }
    if !validation.VerificarSenha(input.SenhaAtual, conta.SenhaHash) {
        return domain.ErrSenhaIncorreta
    }
    if input.NovaSenha == input.SenhaAtual { return domain.ErrValorInvalido }
    if !validation.ValidadorSenha(input.NovaSenha) { return domain.ErrSenhaCurta }

    hash, err := validation.HashSenha(input.NovaSenha)
    if err != nil { return fmt.Errorf("erro hash: %w", err) }
