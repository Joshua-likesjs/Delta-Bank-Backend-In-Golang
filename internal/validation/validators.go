...
// ============================================================
//  REGEX — compiladas uma única vez no startup (FIX 2)
// ============================================================

var (
	reNaoNumericos = regexp.MustCompile(`[^0-9]`)
...
func ValidadorEmail(email string) (string, bool) {
	...
	// FIX 2: regex pré-compilada; FIX 3: checagem redundante removida
...
// GerarChaveAleatoria gera UUID v4 criptograficamente seguro para chave PIX
// FIX 1: substituído LCG com seed de tempo por crypto/rand (seguro e entropia total)
func GerarChaveAleatoria() string {
	u := make([]byte, 16)
	if _, err := rand.Read(u); err != nil {
		panic(fmt.Sprintf("erro ao gerar chave aleatoria: %v", err))
	}
	// Ajustes de versão e variante conforme RFC 4122
	u[6] = (u[6] & 0x0f) | 0x40 // versão 4
...
func removerNaoNumericos(s string) string {
	return reNaoNumericos.ReplaceAllString(s, "") // FIX 2: usa regex pré-compilada
}
